...
type ReadClient interface {
	...
	LeaseEvents(context.Context, mtypes.LeaseID, bool) (ctypes.EventsWatcher, error)
...
func (c *nullClient) LeaseEvents(ctx context.Context, lid mtypes.LeaseID, follow bool) (ctypes.EventsWatcher, error) {
	c.mtx.Lock()
	defer c.mtx.Unlock()
	...
	go func() {
		...
		send := func(evt *eventsv1.Event) bool {
			select {
			case <-feed.Done():
				return false
			case <-lease.ctx.Done():
				return false
			case feed.SendChan() <- evt:
				return true
			}
		}
